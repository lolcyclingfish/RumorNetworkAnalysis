...
#load data:
rawdat<-read.csv("politifact.csv")
network_table <- read.csv("pol_agg.csv")

#write facebook network to csv:
network_fb<-network_table[network_table$Source=="www.facebook.com" | network_table$website=="www.facebook.com",]
write.csv(network_fb,"facebook_network.csv")

#EDA
a<-as.vector(network_table[,1])
b<-as.vector(network_table[,2])
count_news<-c(a,b)
length(unique(count_news)) #905 distinct news

########extract network table###########
#Fake news network table
...
#True news network table
network_table2 = network_table[,c(1,2,4)]
network_table2= network_table2[network_table2$TRUE.>0,]
names(network_table2)[3]<-"CoOccurrences"
#Total news network table
network_table3 = network_table[,c(1,2,3)]
network_table3= network_table3[network_table3$page_url>0,]
names(network_table3)[3]<-"CoOccurrences"
#facebook as source/destination news network table
network_table4 = network_table[,c(1,2,3)]
network_table4= network_table4[network_table4$Source=="www.facebook.com" | network_table4$website=="www.facebook.com",]
names(network_table4)[3]<-"CoOccurrences"



#total_table for fake news network
...
#total_table for true news network
x1<-as.data.frame(network_table2$Source)
x2<-as.data.frame(network_table2$website)
names(x1)<-"nodes"
names(x2)<-"nodes"
total_table2<-rbind(x1,x2)
nrow(total_table2) #198
total_table2<-unique(total_table2) #
nrow(total_table2) #88
#total_table for total news network
x1<-as.data.frame(network_table3$Source)
x2<-as.data.frame(network_table3$website)
names(x1)<-"nodes"
names(x2)<-"nodes"
total_table3<-rbind(x1,x2)
nrow(total_table3) #2904
total_table3<-unique(total_table3) #
nrow(total_table3) #905
#total_table for facebook news network
x1<-as.data.frame(network_table4$Source)
x2<-as.data.frame(network_table4$website)
names(x1)<-"nodes"
names(x2)<-"nodes"
total_table4<-rbind(x1,x2)
nrow(total_table4) #354
total_table4<-unique(total_table4) #
nrow(total_table4) #175
...
#true news network:
g2_valued <- graph_from_data_frame(d = network_table2,directed = TRUE,vertices = total_table2)
#total news network:
g3_valued <- graph_from_data_frame(d = network_table3,directed = TRUE,vertices = total_table3)
#facebook news network:
g4_valued <- graph_from_data_frame(d = network_table4,directed = TRUE,vertices = total_table4)
...
#true news network
numVertices <- vcount(g2_valued)
numVertices #88
numEdges <- ecount(g2_valued)
numEdges #99

#facebook news network
numVertices <- vcount(g4_valued)
numVertices #175
numEdges <- ecount(g4_valued)
numEdges #177
...
graphDensity <- graph.density(g2_valued)
graphDensity #0.001863694

graphDensity <- graph.density(g4_valued)
graphDensity #0.005812808
...
centralization.degree(g2_valued,normalized = TRUE) #0.1671291
centralization.degree(g4_valued,normalized = TRUE) #0.5085877

# Betweenness centralization of split graphs
centralization.betweenness(g_valued,normalized = TRUE) #0.03434465
centralization.betweenness(g2_valued,normalized = TRUE) #0.007437006
centralization.betweenness(g4_valued,normalized = TRUE) #0.0498638

# Closeness centralization of split graphs
centralization.closeness(g_valued,normalized = TRUE) #0.0008192955
centralization.closeness(g2_valued,normalized = TRUE) #0.01044297
centralization.closeness(g4_valued,normalized = TRUE) #0.1145333


# Eigenvector centralization of split graphs
centralization.evcent(g_valued,normalized = TRUE) #0.9766616
centralization.evcent(g2_valued,normalized = TRUE) #0.9440524
centralization.evcent(g4_valued,normalized = TRUE) #0.9327255
...
#for facebook news network
deg_distr4 <-degree.distribution(g4_valued, cumulative=T, mode="all")
...
power4 <- power.law.fit(deg_distr4)
power4
...
plot(deg_distr, log="xy", ylim=c(.001,1), bg="black",pch=21, xlab="Degree", ylab="Cumulative Frequency",
     main="Degree distribution plot of fake news network")

plot(deg_distr4, log="xy", ylim=c(.001,1), bg="black",pch=21, xlab="Degree", ylab="Cumulative Frequency",
     main="Degree distribution plot of facebook network")

########testing against small-world network attributes:
# Average clustering coefficient (ACC)
transitivity(g_valued, type = c("average")) #0.07016435
transitivity(g4_valued, type = c("average")) #0

# Characteristic path length (CPL)
average.path.length(g_valued) #4.016522
average.path.length(g4_valued) #1.895
